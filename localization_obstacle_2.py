...
obstacles = [
    {'position': (15, 15), 'width': 10, 'height': 5},
    {'position': (30, 35), 'width': 5, 'height': 10},
    {'position': (40, 10), 'width': 5, 'height': 5}
]
...
for obs in obstacles:
    ...
    obstacle_polygons.append(box(x, y, x + w, y + h)) # Shapely polygons
...
def cost_function(candidate_position, worker_positions, robot_position, weights):
    ...
    for worker_pos in worker_positions:
        ...
        line_of_sight = LineString([tuple(candidate_position), tuple(worker_pos)])
        obstructed = False
        for obstacle in obstacle_polygons:
            if line_of_sight.crosses(obstacle) or line_of_sight.within(obstacle):
                visibility_cost += 1
                obstructed = True
                break
    ...
    proximity_cost = np.sum(distances_to_workers)  # Sum of distances to all workers
...
for obs in obstacles:
    x, y = obs['position']
    w, h = obs['width'], obs['height']
    rect = Rectangle((x, y), w, h, color='gray', alpha=1)
    ax.add_patch(rect)
...
ax.set_title('Robot Position Optimization with Obstacles')
ax.legend()
ax.grid(True)
plt.show()